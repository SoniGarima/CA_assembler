    ...
    def pipeline(self):
        while(self.PC in pc_dict):
            ...
            if(self.Cycles == 1):
                ...
                self.data.append(self.ID['operands'][len(self.ID['operands'])-1])
                self.fetch(pc_dict[self.PC])
                self.Cycles+=1
                self.total_cycles+=1
            if(self.Cycles == 2):
            ...
            if(self.Cycles == 3):
                ...
                if(self.stall_one or self.stall_two):
                    ...
                    print("----------Stalling the pipeline due to data dependency-----------")
                    self.Cycles+=1
                    self.total_cycles+=1
                    return
            ...
            if(self.Cycles >= 4):
                ...
                for op in self.ID['operands'][:len(self.ID['operands'])-1]:
                    if (op == self.data[len(self.data)-2]):
                        self.stall_one = True
                    if(op == self.data[len(self.data)-3]):
                        self.stall_two = True  
                if(self.stall_one or self.stall_two):
                    self.Cycles+=1
                    self.total_cycles+=1
    ...
    def decode(self,Ins):
        ...
        if(opcode=="0110011"):
            ...
            if(fun3=="000"):
                ...
                out['operands'] = [rs1,imm,rd] 
                if(imm[0]=='1'):
                    eff_address=my_reg.regVal(int(imm,2))-2**12+int(imm,2)
                else:    
                    eff_address=my_reg.regVal(int(rs1,2))+int(imm,2)  
                self.PC=eff_address
        elif(opcode=="0100011"):
            fun3=curr_bin_ins[17:20]
            rs1=curr_bin_ins[12:17]
...
print("--------------------------------------------------------------")
print("NO. of Cycles: ",my_pipeline.total_cycles)  
print("------------------Registers------------------------------\n\n")
mr=my_reg.ret_reg()
for i in range(0,32):
